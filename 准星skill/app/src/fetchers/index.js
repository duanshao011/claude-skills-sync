...
export async function fetchBlogger(blogger) {
  ...
  try {
    for (const a of articles) {
      ...
      if (existing) {
        // 存量文章若缺 work_uuid，补全（公众号正文查询依赖它）
        if (a.workUuid && !db.get('SELECT work_uuid FROM articles WHERE id = ?', [existing.id]).work_uuid) {
          db.run('UPDATE articles SET work_uuid = ? WHERE id = ?', [a.workUuid, existing.id]);
        }
        continue;
      }
      db.run(
        'INSERT INTO articles (blogger_id, title, url, summary, thumbnail, published_at, work_uuid) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [blogger.id, a.title, a.url, a.summary, a.thumbnail, a.published_at, a.workUuid || null]
      );
