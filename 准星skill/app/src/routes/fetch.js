...
import { searchGongzhonghao, verifyGongzhonghao } from '../fetchers/redfox.js';
...
// Search 公众号 accounts by name (must be before /:id)
router.post('/search-gzh', async (req, res) => {
  const { keyword } = req.body;
  if (!keyword || !keyword.trim()) {
    return res.status(400).json({ error: '请输入公众号名称' });
  }
  try {
    const accounts = await searchGongzhonghao(keyword.trim());
    res.json({ accounts });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// Verify a 公众号 is covered by RedFox before adding (must be before /:id)
router.post('/verify-gzh', async (req, res) => {
  const { accountName } = req.body;
  if (!accountName || !accountName.trim()) {
    return res.status(400).json({ error: '缺少公众号名称' });
  }
  try {
    const result = await verifyGongzhonghao(accountName.trim());
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
