...
// Migrate existing DBs: ensure work_uuid / content columns exist on articles
// (sql.js Database has no .all(); read columns via prepare)
const colStmt = db.prepare(`PRAGMA table_info(articles)`);
const articleCols = [];
while (colStmt.step()) articleCols.push(colStmt.getAsObject().name);
colStmt.free();
if (!articleCols.includes('work_uuid')) {
  db.exec(`ALTER TABLE articles ADD COLUMN work_uuid TEXT`);
}
if (!articleCols.includes('content')) {
  db.exec(`ALTER TABLE articles ADD COLUMN content TEXT`);
}
if (!articleCols.includes('title_cn')) {
  db.exec(`ALTER TABLE articles ADD COLUMN title_cn TEXT`);
}
if (!articleCols.includes('summary_cn')) {
  db.exec(`ALTER TABLE articles ADD COLUMN summary_cn TEXT`);
}
scheduleSave();
