...
**摘要必须基于原文生成。信息不足时说明情况，不做任何推测。**

这是硬约束，不是建议。曾出现过的事故：抓取切到广域库后正文链路失效，系统拿
「标题 + 一句话来源描述」去让模型发挥，产出四段结构完整的分析——读起来像模像样，
实际全是编的。用户无法分辨哪些是真的。
...
|---|---|
| 正文 ≥ 200 字 | 正常生成摘要 |
| 正文 < 200 字或取不到 | **不调用模型**，直接返回「内容不全，无法总结」并说明可能原因 |
| 数据源返回的文章与请求的不一致 | 拦截丢弃，按「取不到」处理 |
| 内容不全的结果 | **不写入缓存**，等正文可用时用户再点即可拿到真摘要 |

正文最低长度阈值 `MIN_CONTENT_CHARS = 200`。低于此值认为拿到的只是标题或零星片段。
历史上该阈值为 50，导致 89 字的纯图片文章也被拿去生成了四段分析。

**数据源校验**：红狐 `queryArticleDetail` 按 URL 直查会偶发返回另一篇文章，且返回内容
不稳定（同一 URL 两次请求返回了两篇不同的无关文章）。落库前必须比对标题，对不上宁可不要——
正文错配比抓不到严重得多，会让摘要一本正经地总结别人的文章。
