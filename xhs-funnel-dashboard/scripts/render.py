...
COLUMN_GROUPS = [
    {
        ...
        "columns": [
            {"key": "note_id",    "label": "笔记ID",     "unit": "",   "type": "id",
             "formula": "小红书笔记唯一 ID", "meaning": "五表关联的主键", "source": "五表"},
            {"key": "creator",    "label": "达人昵称",   "unit": "",   "type": "text",
             "formula": "蒲公英 → 星河 → 薯条；灵犀仅使用非品牌占位昵称兜底",
             "meaning": "笔记真实作者；灵犀中的光粒等品牌账号不回填", "source": "五表身份字段"},
    ...
    },
    {
        ...
        "columns": [
            ...
            {"key": "gmv", "label": "商家GMV", "unit": "元", "type": "num",
             "formula": "星河『商家GMV』", "meaning": "该笔记带来的成交金额（⚠ 多内容归因下同一订单GMV会重复计入不同笔记，加总后非精确值）", "source": "星河"},
            {"key": "uv_value", "label": "进店UV价值", "unit": "元", "type": "num",
             "formula": "商家GMV / 进店UV",
             "meaning": "每个进店用户平均带来的成交金额，越高说明进店质量越好（GMV含多内容归因重复，该值为近似参考）", "source": "系统计算"},
    ...
    },
    {
        ...
        "label": "投放汇总",
        "columns": [
            {"key": "spend", "label": "总投放金额", "unit": "元", "type": "num",
             "formula": "薯条实际支付 + 聚光实际消耗",
             "meaning": "该笔记两渠道全量投入；若某渠道未上传，页面会明确提示口径不完整", "source": "系统计算"},
            {"key": "paid_days", "label": "总投放天数", "unit": "天", "type": "int",
             "formula": "薯条启动日 ∪ 聚光消费日，日期去重",
             "meaning": "任一渠道发生有效付费的自然日数，同日双投只计1天", "source": "系统计算"},
            {"key": "roi", "label": "ROI", "unit": "", "type": "num",
             "formula": "首个付费日至星河最新日的同样本GMV / 有效合计成本", "meaning": "每花1元有效成本带来的成交金额；GMV含多内容归因重复，仅作相对比较", "source": "系统计算", "needs": ["星河"]},
            {"key": "uv_cost", "label": "阅读UV成本", "unit": "元", "type": "num",
             "formula": "有效合计成本 / 星河阅读播放UV", "meaning": "拉动一个星河阅读/播放用户的合计成本，越低越好", "source": "系统计算", "needs": ["星河"]},
            {"key": "visit_uv_cost", "label": "进店UV成本", "unit": "元", "type": "num",
             "formula": "有效合计成本 / 星河进店UV", "meaning": "拉动一个进店用户的合计成本，越低越好", "source": "系统计算", "needs": ["星河"]},
            {"key": "cart_cost", "label": "加购成本", "unit": "元", "type": "num",
             "formula": "有效合计成本 / 星河加购UV", "meaning": "拉动一个加购用户的合计成本，越低越好", "source": "系统计算", "needs": ["星河"]},
            {"key": "deal_cost", "label": "成交成本", "unit": "元", "type": "num",
             "formula": "有效合计成本 / 星河成交UV", "meaning": "拉动一单成交的合计成本，越低越好", "source": "系统计算", "needs": ["星河"]},
        ],
    },
    {
        "key": "chili",
        "label": "薯条投放",
        "columns": [
            {"key": "chili_spend", "label": "薯条实付", "unit": "元", "type": "num",
             "formula": "薯条实际支付金额，仅推广完成", "meaning": "该笔记薯条渠道全量实付", "source": "薯条"},
            {"key": "chili_days", "label": "薯条投放天数", "unit": "天", "type": "int",
             "formula": "薯条不同启动日去重", "meaning": "薯条渠道实际有投放的天数", "source": "薯条"},
            {"key": "chili_orders", "label": "薯条订单数", "unit": "单", "type": "int",
             "formula": "推广完成订单去重计数", "meaning": "该笔记薯条推广完成订单数", "source": "薯条"},
        ],
    },
    {
        "key": "juguang",
        "label": "聚光投放",
        "columns": [
            {"key": "juguang_spend", "label": "聚光实际消耗", "unit": "元", "type": "num",
             "formula": "聚光日维度明细『消费』按笔记ID汇总", "meaning": "该笔记聚光渠道全量实际消耗", "source": "聚光"},
            {"key": "juguang_days", "label": "聚光投放天数", "unit": "天", "type": "int",
             "formula": "聚光消费>0的日期去重", "meaning": "聚光渠道实际有消费的自然日数", "source": "聚光"},
            {"key": "juguang_impression", "label": "聚光展现量", "unit": "", "type": "int",
             "formula": "聚光原表『展现量』汇总", "meaning": "聚光广告展现次数", "source": "聚光"},
            {"key": "juguang_click", "label": "聚光点击量", "unit": "", "type": "int",
             "formula": "聚光原表『点击量』汇总", "meaning": "聚光广告点击次数", "source": "聚光"},
        ],
    },
    {
        "key": "lx",
...
DEFAULT_COLUMNS = [
    ...
    # 投放汇总 + 渠道拆分
    "spend", "roi", "uv_cost", "chili_spend", "juguang_spend",
    # 灵犀（人群资产）
    "ti_visit_ratio",
]
...
NOTE_FIELDS = [
    ...
    "pub_date",
    "in_pgy", "in_star", "in_chili", "in_juguang", "in_lx",
    "spend", "chili_spend", "juguang_spend", "paid_days", "total_amount", "gmv", "roi", "uv_value", "uv_cost",
    ...
    "chili_max_daily", "chili_days", "chili_orders",
    "juguang_days", "juguang_impression", "juguang_click", "juguang_interaction",
    "effective_spend", "effective_chili_spend", "effective_juguang_spend",
